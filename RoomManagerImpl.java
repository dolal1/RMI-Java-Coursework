    ...
    public ArrayList<String> revenue() throws RemoteException {
        ...
        for (String key : rooms.keySet()) {
            if (key.equals("type0")) {
                ...
                revenueList.add(this.rooms.get(key).size()+" booked rooms of type 0 collected "+(55000 * this.rooms.get(key).size())+" UGX.");
            }
            if (key.equals("type1")) {
               ;
                revenueList.add(this.rooms.get(key).size()+" booked rooms of type 1 collected "+(55000 *  75000 * this.rooms.get(key).size())+" UGX.");
            }
            if (key.equals("type2")) {
              
                revenueList.add(this.rooms.get(key).size()+" booked rooms of type 2 collected "+(80000 * this.rooms.get(key).size())+" UGX.");
            }
            if (key.equals("type3")) {
  
                revenueList.add(this.rooms.get(key).size()+" booked rooms of type 3 collected "+(150000 * this.rooms.get(key).size())+" UGX.");
            }
            if (key.equals("type4")) {
                
                revenueList.add(this.rooms.get(key).size()+" booked rooms of type 4 collected "+(230000 * this.rooms.get(key).size())+" UGX.");
            }
        }
        return revenueList;
    }
